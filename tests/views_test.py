...
class ViewsTest(unittest.TestCase):
    ...
    @patch("app.views.get_followings_ids")
    @patch("app.views.get_posts_to_show")
    @patch("app.views.UserInfo")
    def test_user_page_if_exists_current_user_page(self,
                                                   mock_user_info,
                                                   mock_get_posts_to_show,
                                                   mock_get_followings_ids):
        mock_user_info.get_current_user_userid.return_value = \
            "current_user_userid"
        ...
        mock_get_followings_ids.return_value = [MockRedis.TEST_USER_2]

        rv = self.test_client.get("/current_user_userid")
